...
> [!NOTE]
> 在线版部署目前受到 GitHub Actions 账号状态限制影响，可能暂时无法访问。桌面版和 Release 下载不受影响。
...
合并包含 [`.github/workflows/deploy-pages.yml`](.github/workflows/deploy-pages.yml) 的 PR 后，**首次 push 到 `master` 会自动启用 GitHub Pages**（workflow 会在 deploy 前检查并创建 Pages 站点，通常无需手动进入 Settings）。

若 workflow 中 `enable-pages` 步骤因组织策略等原因失败，仓库维护者可手动执行（与 workflow 等价）：
...
在线地址（资源路径为 `/lys-refiner/`，与 `GITHUB_PAGES=true` 构建一致）：

- 上游：[https://watercooling480.github.io/lys-refiner/](https://watercooling480.github.io/lys-refiner/)
